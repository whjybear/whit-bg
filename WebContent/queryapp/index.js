...
// 初始化放大插件
const zooming = new Zooming({bgColor:'black'});
...
/**
 * 根据证书数据在页面中创建img元素并绘制
 * @param {Array} dataArr 证书数据数组
 */
function createImgDOM (dataArr) {
    let infoContent = document.getElementById('info-content');
    let childes = infoContent.childNodes;
    for (let i = childes.length - 1; i >= 0; i--) {
        childes[i].remove();
    }
    ...
    dataArr.forEach(function (item) {
        ...
        textDOM.id = 'query-times';
        textDOM.className='info-item';
        textDOM.innerText = '此证书为真，该证书被查' + item.queryTimes + '次';
        infoContent.appendChild(textDOM);

        imgDOM.width = 292;
        imgDOM.height = 207;
        imgDOM.className = 'zoom info-item';
        infoContent.appendChild(imgDOM);

        line.className = 'line info-item';
        infoContent.appendChild(line);
        let promiseItem = drawCerImg(item).then(function (base64) {
            imgDOM.src = base64;
        });
    ...
    return Promise.all(promiseArr).then(function () {
        ...
        // document.getElementById('query-times').innerText = '此证书为真，该证书被查' + dataArr[0].queryTimes + '次';
        zooming.listen('.zoom');
    });
}
...
/**
 * 请求接口数据并根据数据绘制图片，显示证书信息
 * @param params 请求接口时携带的参数
 */
function showInfo (params) {
    ...
    fetch(url, {
        ...
        response.json().then(function (data) {
            createImgDOM(data).then(function () {
                // 关闭加载动画
                closeLoading();
            });
            // 开始加载字体
